...
function App() {
  ...
  // Server-managed chat session (created lazily when the user opens the chat tab)
  const [sessionId, setSessionId] = useState(() => sessionStorage.getItem('crossbar_session_id'));
  ...
  // Create a fresh server-side chat session and persist it to sessionStorage.
  const createNewSession = useCallback(async () => {
    try {
      const data = await createSession();
      sessionStorage.setItem('crossbar_session_id', data.session_id);
      ...
      setSessionError(
        status === 409
          ? 'You have reached the maximum number of active sessions. Please wait a few minutes and try again.'
  ...
  // Ensure a session exists before chatting (reuses a stored one if still valid).
  const ensureSession = useCallback(async () => {
    const stored = sessionStorage.getItem('crossbar_session_id');
    if (stored) {
  ...
  // Start a new conversation: clear state, drop the previous session, create a new one.
  const startNewConversation = useCallback(async () => {
    setConversationHistory([]);
    setQueryResult(null);
    setExecutionResult(null);
    setQuestion('');
    const prevId = sessionStorage.getItem('crossbar_session_id');
    if (prevId) {
      try { await deleteSession(prevId); } catch { /* session may already be expired */ }
    }
    sessionStorage.removeItem('crossbar_session_id');
    setSessionId(null);
    await createNewSession();
  }, [createNewSession]);

  // Lazily create a session when the user opens the chat tab.
  useEffect(() => {
    if (tabValue === 'query') {
      ensureSession();
    }
  }, [tabValue, ensureSession]);
